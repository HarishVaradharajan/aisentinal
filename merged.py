...
def analyze_case_knn(user_input, data, top_n=3):
    processed_input = preprocess_text(user_input)
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(data['Processed_Keywords'])
...
def analyze():
    ...
    data = load_data()
    all_results = analyze_case_knn(user_query, data)
    generate_charts()

    # Find the maximum relevance score
    if all_results:
        max_score = max(result["Relevance Score"] for result in all_results)
        # Get all predictions that have the max relevance score
        top_results = [result for result in all_results if result["Relevance Score"] == max_score]
    else:
        top_results = []

    return render_template('dashboard.html', result=top_results)
